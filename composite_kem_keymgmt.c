                               ...
                               const unsigned char *tradpriv, size_t tradlen)
{
    EVP_PKEY_CTX *c = EVP_PKEY_CTX_new_from_name(key->libctx, key->info->pq_alg,
                                                 key->pq_propq);
    /* Standardized combos serialize the ML-KEM private key as its 64-byte seed;
     * any other PQ component uses its raw private octet (mirrors the sig family). */
    const char *pqparam = key->info->pq_priv_seed ? OSSL_PKEY_PARAM_ML_KEM_SEED
